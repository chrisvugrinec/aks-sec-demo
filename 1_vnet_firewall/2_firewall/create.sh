...
function createFirewall(){
   public_ip_id=$(az network public-ip show -n $pip_fw_name -g $resource_group --query id -o tsv)
   mgmt_vnet_id=$(az network vnet show -n $mgmt_vnet_name -g $resource_group --query id -o tsv)
   az deployment group create -g $resource_group --template-file ./fw.json  --parameters fw_name=$fw_name public_ip_id=$public_ip_id mgmt_vnet_id=$mgmt_vnet_id
}
